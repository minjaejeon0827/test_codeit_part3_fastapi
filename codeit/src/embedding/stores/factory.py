...
팩토리 메소드 패턴(Factory Method Pattern):
    - vector_store_type 문자열만 받으면 적절한 Store 객체 생성 및 반환
...
def create_vector_store(config: Dict) -> BaseVectorStore:
    ...
    * 사용 예시:
    ...
    Args:
        config: 프로젝트 설정 딕셔너리
        vector_store_type: 'faiss', 'chroma', 'pinecone', 'qdrant', 'weaviate' 등
    ...
    """
    # vector_store_type = vector_store_type.lower().strip()
    vector_store_type = config["embedding"]["vector_store_type"].lower().strip()
    
    if vector_store_type not in _STORES:
        supported = list(_STORES.keys())
        raise ValueError(
            f"❌ 지원하지 않는 Vector DB 타입: '{vector_store_type}'. "
            f"지원되는 타입: {supported}"
        )
    
    store_class = _STORES[vector_store_type]
    return store_class()